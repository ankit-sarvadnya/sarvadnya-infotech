...
| `CustomerReviews` | Displays verified social proof. | Staggered entrance animations, memoized star ratings. **2026-09-17:** sm & below redesigned — one large card per viewport (scroll-snap, swipe + teal-pill dots + gentle autoplay paused on touch; respects `prefers-reduced-motion`); `ReviewCard` variant: `feature` (large Playfair serif quote + oversized `"` glyph) vs `marquee` (compact 320px, desktop only). Desktop marquee untouched. |
| `CertifiedPartners` | Showcases network reliability. | **2026-09-17 rev-6 (current)** — back to the **ORIGINAL row structure/sizing** with colorful connectors kept: all 5 logos (including TallyPrime) sit in ONE straight line, evenly spread by `linePos()` (x = 10/30/50/70/90, y = 50), on the section gradient (no white container/stage). Original card styling restored — bigger responsive cards (`w-14→w-48` / `h-12→h-32`, xl = 192×128px), white bg, `border-[#E9F1FA]`, name under logo, shimmer + `hover:-translate-y-2`, section `py-16 md:py-20`, title `mb-12`. **TallyPrime container is one size step LARGER** (xl 224×144px) and sits center (index 2) acting as the hub. **2026-09-17 rev-12 (current)** — Two responsive layouts, latest = stability + mobile equal-size. **Desktop (md+):** all 5 partners in ONE straight line, TallyPrime slightly UPPER (`HUB_Y 42` vs `BRAND_Y 52`, percentage-anchored) & one size larger; connectors are DEEP **elliptical** under-loops (`LOOP_DEPTH 128`, control spread `0.30`, apex ≈ 118.3 below stage, SVG `overflow-visible`). **Mobile (sm & below):** **2-1-2 star** — 2 brand cards top row (x 18/82, y 20), TallyPrime CENTRAL, 2 brand cards bottom row (y 80). Mobile cards are completely STATIC (no hover handlers, no `:hover` lift, connectors ignore hovered) so the entrance animation runs once and nothing retriggers up/down-scale jitter; desktop hover is gated behind `matchMedia('(hover:hover) and (pointer:fine)')` (`canHover`) to kill sticky-hover/state races on touch. **Mobile sizes (rev-12):** ALL FIVE cards are the **same size** — TallyPrime is no longer larger; every card is `w-24 h-[72px]`/`sm:w-32 sm:h-24` (96×72 / 128×96), brands grown from 72×56. Slots splayed outward to x 18/82 and stage raised to `h-48 sm:h-64` so the equal-width cards clear the hub (3.84px side gaps, 2.4px top/bottom margin at 360px). Cards SQUARISH `rounded-md` vs desktop `rounded-2xl`; section `py-10 md:py-14`. Brand-colored connectors (`BRAND_STROKES` — Biz Analyst `#38bdf8`, OTU `#f59e0b`, AWS `#f97316`, NoSky `#0f172a`; unknown fall back to NoSky) with `.connect-flow` dash animation in `globals.css`. HomeHero QuickAccess cards: mobile icon `w-11`, `lg:mt-2` icon→title gap on the `lg:block` desktop layout. Parent wrapper divs, section gradient, eyebrow line and H2 are unchanged. |
...
| `News` (`/news`, `/news/[slug]`) | SEO blog over the shared MongoDB `news` collection. Legacy admin posts (title/date/category/description/content/link) are auto-enriched into indexable articles — explicit `slug`/`seoTitle`/`tags`/`author` win when present, otherwise derived. Listing cards link internally (`/news/{slug}`, excerpt, reading time). Article page: force-dynamic, per-article metadata, author/date/reading-time header, bullet-aware body renderer, tags, 3 related posts, CTA + JSON-LD (NewsArticle + breadcrumb). `/sitemap.xml` appends article URLs (18 live). Search API news hits → `/news/{slug}`. | `lib/news-utils.ts` (`enrichNews`, `resolveSlug`, `idTail` — stringify ObjectId before slicing, else `b.slice is not a function`), `lib/news.ts` (`NewsItem` + optional blog fields), `getNewsBySlug`. Seeded by `scripts/seed_news.mjs` (15 keyword-targeted posts, idempotent upsert by slug, **ASCII-only content** — an em-dash write once corrupted the file and dropped closing backticks; `tags` are part of the upsert comparison; `REMOVE_SLUGS` deletes seeded slugs that are dropped from the file, keeping the blog in sync with the seed and letting removed posts leave `/news`, `/sitemap.xml` and the search API). **2026-09-07:** +6 local-keyword posts (5-star partner, near-me, HRMS, TSS expiry meaning, cloud access, CBD Belapur list) and retitled 2 weak posts for CTR (`tally-erp9-add-ons`, `tally-partner-mumbai`). **2026-09-16:** removed `tally-dealers-mumbai` + scrubbed all "dealer" tags per owner — news stays professional (partner framing, no reseller branding); 18 docs live. **2026-09-17:** TSS-renewal body copy no longer limits coverage to specific cities; now says "anywhere in India" per owner (local stays in titles/keywords only). |
