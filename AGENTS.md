...
| `News` (`/news`, `/news/[slug]`) | SEO blog over the shared MongoDB `news` collection. Legacy admin posts (title/date/category/description/content/link) are auto-enriched into indexable articles — explicit `slug`/`seoTitle`/`tags`/`author` win when present, otherwise derived. Listing cards link internally (`/news/{slug}`, excerpt, reading time). Article page: force-dynamic, per-article metadata, author/date/reading-time header, bullet-aware body renderer, tags, 3 related posts, CTA + JSON-LD (NewsArticle + breadcrumb). `/sitemap.xml` appends article URLs (19 live). Search API news hits → `/news/{slug}`. | `lib/news-utils.ts` (`enrichNews`, `resolveSlug`, `idTail` — stringify ObjectId before slicing, else `b.slice is not a function`), `lib/news.ts` (`NewsItem` + optional blog fields), `getNewsBySlug`. Seeded by `scripts/seed_news.mjs` (16 keyword-targeted posts, idempotent upsert by slug, **ASCII-only content** — an em-dash write once corrupted the file and dropped closing backticks). **2026-09-07:** +6 local-keyword posts (5-star partner, near-me, HRMS, TSS expiry meaning, cloud access, CBD Belapur list) and retitled 3 weak posts for CTR (`tally-dealers-mumbai`, `tally-erp9-add-ons`, `tally-partner-mumbai`). |
