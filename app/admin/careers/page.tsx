    ...
    <div>
      ...
      <div className="flex gap-4 mb-8">
        <button 
          ...
          className={`px-6 py-2 rounded-xl font-bold transition-all ${activeTab === 'listings' ? 'bg-[#006569] text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
        >
          Job Listings
        </button>
        <button 
      ...
      {editingJob ? (
        <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm mb-10">
          ...
          <form onSubmit={handleSave} className="space-y-6">
            ...
            <div className="flex gap-4">
              <button type="submit" className="bg-[#006569] text-white px-8 py-3 rounded-2xl font-bold">Save Job</button>
        ...
        <div className="grid grid-cols-1 gap-4">
          {loading ? (
            ...
            jobs.map(job => (
              <div key={job.id || (job as any)._id} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex justify-between items-center group hover:border-[#006569]/30 transition-all">
                <div>
                ...
                {loading ? (
                  ...
                  applications.map((app) => (
                    <tr key={app._id} className="border-b border-slate-50 hover:bg-slate-50/30 transition-colors">
                      ...
                      <td className="p-4">
                        <div className="text-sm font-black text-[#006569] tabular-nums">{app.phone}</div>
                      </td>
                      <td className="p-4 text-sm font-medium text-slate-600">
                        {app.job_title}
                      </td>
                      <td className="p-4">
                         <span className="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-sky-50 text-[#006569] border border-sky-100">
                           {app.experience}
                         </span>
                      </td>
