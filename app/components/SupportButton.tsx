...
export default function SupportButton({ initialSettings }: { initialSettings?: any }) {
  ...
  return (
    <>
      {/* Ask Sara — right on mobile (above WhatsApp), right on desktop */}
      <button
      ...
      {/* WhatsApp — right on mobile (below Sara), left on desktop */}
      <a
        href={`https://wa.me/${whatsappNumber}`}
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-20 right-6 sm:bottom-6 sm:left-6 sm:right-auto z-3000 flex items-center gap-2 bg-[#25D366] text-white px-4 py-2.5 rounded-full shadow-[0_10px_30px_rgba(37,211,102,0.2)] hover:shadow-[0_15px_40px_rgba(37,211,102,0.3)] transition-all duration-500 ease-in-out hover:-translate-y-1 active:scale-95 group"
      >
